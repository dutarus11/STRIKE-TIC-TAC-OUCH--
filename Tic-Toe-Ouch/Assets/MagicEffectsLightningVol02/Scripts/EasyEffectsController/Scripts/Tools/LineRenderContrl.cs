// (c) Copyright 2014-2015 Luke Light&Magic. All rights reserved.
...
public class LineRenderContrl : MonoBehaviour {
  ...
  void Awake() {
    ...
    if (mat) {
      ...
      //Debug.Log(sheetCoordinates.Length);
  ...
  //  mat.SetTextureScale("_MainTex", new Vector2(1, 1/TexSheetCount));
  //  AnimationCurve offsetCurve = AnimationCurve.Linear(0, 0, TexMovertime, 1);
  //  AnimationClip clip = new AnimationClip();
  //  clip.SetCurve("", typeof(Material), "_MainTex.offset.x", offsetCurve);

  //  AnimationCurve sheetCurve = new AnimationCurve();
  //  Keyframe key = new Keyframe();
  //  key.inTangent = float.PositiveInfinity;
  //  key.outTangent = key.inTangent;
  //  for (var i = 0; i <= TexSheetCount; i++) {
  //    key.time = TexSheetTotalTime / TexSheetCount * i;
  //    key.value = (i + 1) / TexSheetCount;
  //    sheetCurve.AddKey(key);
  //  }
  //  clip.SetCurve("", typeof(Material), "_MainTex.offset.y", sheetCurve);

  //  clip.wrapMode = WrapMode.Loop;
  //  animation.clip = clip;
  //  animation.clip.name = "tex_move";
